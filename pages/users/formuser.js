...
export default function FormUser(){
    ...
    const handleSubmit = ()=>{
        ...
        NewUser(query).then((resp)=>{
            console.log('========RESPUESTA NEW USER', resp)
            setIsLoading(false)
        })
    }
    useEffect(()=>{
    ...
    return(
        ...
        <div className='flex flex-col w-full'>
            ...
            <input name='ayn' placeholder='Nombre y Apellido' required className='w-11/12 focus:outline-none focus:ring-2 focus:ring-blue-300 rounded-xl p-3 mx-3 my-2' onChange={handleChange()} value={query.ayn} />
            <input name='email' type='email' placeholder='E-Mail' required className='w-11/12 focus:outline-none focus:ring-2 focus:ring-blue-300 rounded-xl p-3 mx-3 my-2' onChange={handleChange()} value={query.email} />
            <input name='password' type='password' placeholder='Password' required className='w-11/12 focus:outline-none focus:ring-2 focus:ring-blue-300 rounded-xl p-3 mx-3 my-2' onChange={handleChange()} value={query.password} />
            <div className="relative w-11/12 mx-3">
                ...
                <select name='rol' required className="border border-gray-200 rounded-xl text-gray-600 h-12 pl-3 pr-10 my-2 w-full bg-white focus:ring-2 focus:ring-blue-300 focus:outline-none appearance-none" onChange={handleChange()}>
                    <option disabled selected hidden>Seleccione el rol...</option>
            ...
            <div className="relative w-11/12 mx-3">
                ...
                <select name='id_area' required className="border border-gray-200 rounded-xl text-gray-600 h-12 pl-3 pr-10 my-2 w-full bg-white focus:ring-2 focus:ring-blue-300 focus:outline-none appearance-none" onChange={handleChange()}>
                    <option disabled selected hidden>Seleccione el área...</option>
                    {areas && areas.map((item)=><option key={item.id} value={item.id}>{item.nombre}</option>)}
                </select>
            </div>
            }
