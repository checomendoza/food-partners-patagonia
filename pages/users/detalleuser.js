			...
			{isLoading ? (
				...
				<div className="w-full flex flex-col items-center">
					<form
						...
						className="w-full flex flex-col lg:items-center lg:w-6/12"
					>
						<input
							name="ayn"
						...
						<div className="relative w-11/12 mx-3">
							...
							>
								<option value="Admin" selected={query.rol == "Admin"}>
									Administrador
								</option>
								<option value="Operador" selected={query.rol == "Operador"}>
									Operador
								</option>
							</select>
						</div>
						{query.rol == "Operador" && (
							<div className="relative w-11/12 mx-3">
								...
								>
									{areas &&
										areas.map((item) => (
											<option key={item.id} value={item.id} selected={item.nombre == userEdit.nombre}>
												{item.nombre}
											</option>
										))}
								</select>
						...
						<div className="flex flex-col items-center w-full">
							...
							<button
								type="submit"
								className="w-7/12 p-3 bg-transparent border border-red-500 text-red-500 text-xl mt-7 mb-3 mx-3 rounded-xl focus:outline-none"
